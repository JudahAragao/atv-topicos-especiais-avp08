...
import { getFirestore, doc, onSnapshot } from 'firebase/firestore'; // Importa as funções necessárias do Firebase 9
...
export default function CreateSchedule({ navigation, route }) {
    ...
    const [state, setState] = React.useState({
        ...
        idService: '',
        priceServico: '',
        idUser: global.idUsuario, //Inicializa o estado com as variáveis globais
        userName: global.nomeUsuario,
        userEmail: global.emailUsuario,
        userPhone: global.telefoneUsuario,
    });
    ...
    React.useEffect(() => {
        ...
        const unsubscribeFormasPagamento = onSnapshot(doc(db, 'formPayment'), (snapshot) => {
            const formasPagto = snapshot.docs.map((doc) => ({
                label: doc.data().descricao,
                value: doc.id,
            }));
            setformasPagto(formasPagto);
        });

        const unsubscribeServicos = onSnapshot(doc(db, 'Services'), (snapshot) => {
            const servicos = snapshot.docs.map((doc) => ({
                label: `${doc.data().descricao} (R$ ${doc.data().preco.toFixed(2).replace(".", ",")})`,
                value: doc.id,
